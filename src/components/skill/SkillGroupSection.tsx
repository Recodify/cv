...
export default function SkillGroupSection({ title, skills, subGroups, forcePageBreak }: Readonly<Omit<SkillGroup, 'category'>>) {
  ...
  const headerContent = (
    <>
      ...
      <div className="md:hidden text-slate-900 absolute right-4">
    ...
    return (
      <div className={`${forcePageBreak ? 'print:breakbefore' : 'print:nobreak'}`}>
        <div className="space-y-6">
          ...
          <div className={`space-y-4 sm:space-y-10 pb-4 ${!isExpanded ? 'hidden md:block' : ''}`}>
            {subGroups.map((subGroup) => (
              <div key={subGroup.title} className="space-y-2 sm:space-y-3 pb-2">
                ...
                <div className="grid grid-cols-2 gap-y-2 sm:gap-y-3 gap-x-4 px-4 md:px-6 print:pl-6">
  ...
  return (
    <div className={`${forcePageBreak ? 'print:breakbefore' : 'print:nobreak'}`}>
      <div className="space-y-6">
        ...
        <div className={`pb-4 ${!isExpanded ? 'hidden md:block' : ''}`}>
          <div className="grid grid-cols-2 gap-y-2 sm:gap-y-3 gap-x-4 px-4 md:px-6 print:pl-5">
            {skills?.map((skill) => (
              <SkillItem key={skill.name} {...skill} />
            ))}
          </div>
